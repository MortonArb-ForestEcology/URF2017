...
plots.sp <- spTransform(plots.sp, projection(morton.grid))
...
ggplot() +
  ...
  geom_point(data=plot.info, aes(x=x, y=y, color=Description, shape=Type, size=Type)) +
  scale_shape_manual(values=c(15, 19)) + # see ?pch for shapes
  scale_color_manual(values=c("blue2", "green4", "orange3", "red3"), name="Stand") +
